...
def check_uid_availability(
    metagraph: "bt.metagraph.Metagraph",  # type: ignore
    uid: int,
    vpermit_tao_limit: int,
    coldkeys: set | None = None,
    ips: set | None = None,
) -> bool:
...
def get_random_uids(self, k: int, exclude: List[int] | None = None) -> torch.LongTensor:
    ...
    if 0 < len(candidate_uids) < k:
        ...
        return torch.tensor(candidate_uids)  # type: ignore
    elif len(candidate_uids) >= k:
        return torch.tensor(random.sample(candidate_uids, k))  # type: ignore
    else:
